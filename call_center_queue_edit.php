...
//get http post variables and set them to php variables
	if (is_array($_POST)) {
		...
		//remove invalid characters
			...
			$queue_cid_prefix = str_replace("\\", "", $queue_cid_prefix);
			$queue_cid_prefix = str_replace("/", "", $queue_cid_prefix);
		if ($queue_strategy == 'ring-all' && !empty($_SESSION['call_center']['agents_limit']['numeric'])){
			$agents_limit = $_SESSION['call_center']['agents_limit']['numeric'];
		}
		if (!empty($agents_limit) && is_array($_POST["call_center_tiers"]) && @sizeof($_POST["call_center_tiers"]) > $agents_limit ) {
			foreach ($_POST["call_center_tiers"] as $key => $row) {
				if ($key >= $agents_limit){
					$remove_tires[] = $row['call_center_tier_uuid'];
					unset($_POST["call_center_tiers"][$key]);
				}
			}
		}
	}
...
//delete the tier (agent from the queue)
	if (
		($_REQUEST["a"] == "delete" && is_uuid($_REQUEST["id"]) && permission_exists("call_center_tier_delete"))
		|| !empty($remove_tires)
	) {
		if (empty($remove_tires)){
			$remove_tires[] = $_REQUEST["call_center_tier_uuid"];
			$call_center_queue_uuid = $_REQUEST["id"];
		}
		else{
			$call_center_queue_uuid = $_POST["call_center_queue_uuid"];
		}
		foreach ($remove_tires as $remove_tier) {

				$call_center_tier_uuid = $remove_tier;
...
//add an empty row to the tiers array
	...
	if (count($tiers) > 0) {
		...
		if (!empty($agents_limit) &&  count($tiers) >= $agents_limit){
			$rows = 0;
		}
	}
	for ($x = 0; $x < $rows; $x++) {
